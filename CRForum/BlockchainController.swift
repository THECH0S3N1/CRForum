...
class  BlockchainController: UIViewController , UITableViewDelegate, UITableViewDataSource {
    ...
    @IBOutlet weak var blockchainDataTable: UITableView!
    
    
    @IBAction func updateTable(_ sender: Any) {
        blockchainDataTable.reloadData()
    }
    ...
    override func viewDidLoad() {
        ...
        downloadPosts()
        blockchainDataTable.reloadData()
        blockchainDataTable.dataSource = self
        blockchainDataTable.delegate = self
        blockchainDataTable.register(UITableViewCell.self, forCellReuseIdentifier: "BlockCell")
